...
<template>
  <dialog ref="dialogRef" class="modal" @close="handleClose">
    <div v-if="booking" class="modal-box w-11/12 max-w-2xl p-0 bg-base-100 shadow-2xl border border-base-content/10 overflow-hidden flex flex-col max-h-[90vh]">
      <!-- Header Section (Fixed Gradient) -->
      <div class="relative bg-gradient-to-br from-primary/10 via-base-100 to-secondary/5 p-6 md:p-8 border-b border-base-content/5 shrink-0">
      ...
      <div class="p-6 md:p-8 flex-1 overflow-y-auto premium-scroll space-y-6">
        
        <!-- Section: Time & Date -->
        <section class="space-y-3">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded bg-primary/10 flex items-center justify-center text-primary"><Calendar :size="16" /></div>
            <h4 class="text-lg font-black uppercase tracking-tight">วันและเวลา</h4>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex items-center gap-3 p-3 bg-base-200/30 border border-base-content/5">
              <div class="w-8 h-8 rounded bg-primary/10 flex items-center justify-center text-primary"><Calendar :size="16" /></div>
              <div>
                <div class="text-[9px] opacity-40 uppercase font-black">วันที่จอง</div>
                <div class="font-bold text-sm">{{ formatDate(booking.start_time) }}</div>
              </div>
            </div>
            <div class="flex items-center gap-3 p-3 bg-base-200/30 border border-base-content/5">
              <div class="w-8 h-8 rounded bg-secondary/10 flex items-center justify-center text-secondary"><Clock :size="16" /></div>
              <div>
                <div class="text-[9px] opacity-40 uppercase font-black">เวลา</div>
                <div class="font-bold text-sm">{{ formatTime(booking.start_time) }} - {{ formatTime(booking.end_time) }}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="space-y-2">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded bg-neutral/10 flex items-center justify-center text-neutral"><FileText :size="16" /></div>
            <h4 class="text-lg font-black uppercase tracking-tight">วาระการประชุม</h4>
          </div>
          <div class="p-4 bg-base-200/50 border border-base-content/5 relative overflow-hidden group min-h-[80px]">
            <div class="absolute top-0 left-0 w-1 h-full bg-primary opacity-20 group-hover:opacity-100 transition-opacity"></div>
            <p class="text-sm leading-relaxed font-medium text-base-content/80 whitespace-pre-wrap">
        ...
        <section class="space-y-3">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded bg-primary/10 flex items-center justify-center text-primary"><MapPin :size="16" /></div>
            <h4 class="text-lg font-black uppercase tracking-tight">ข้อมูลสถานที่</h4>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="p-4 bg-base-200/50 border border-base-content/5 flex items-center gap-3">
              <div class="w-10 h-10 bg-base-100 flex items-center justify-center shadow-sm">
                <Building2 :size="20" class="opacity-40" />
              </div>
              <div>
                <div class="text-[9px] opacity-40 uppercase font-black tracking-widest">ชื่อห้องประชุม</div>
                <div class="font-bold text-md leading-tight">{{ booking.snap_room_name }}</div>
                <div class="text-[9px] opacity-50 mt-0.5 uppercase">{{ booking.snap_room_location }}</div>
              </div>
            </div>
            <div class="p-4 bg-base-200/50 border border-base-content/5 flex items-center gap-3">
              <div class="w-10 h-10 bg-base-100 flex items-center justify-center shadow-sm">
              ...
              <div>
                <div class="text-[9px] opacity-40 uppercase font-black tracking-widest">จำนวนผู้เข้าร่วม</div>
                <div class="text-lg font-black">{{ booking.attendee_count }} <small class="text-[10px] opacity-40">/ {{ booking.snap_room_capacity }}</small></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="space-y-3">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded bg-secondary/10 flex items-center justify-center text-secondary"><User :size="16" /></div>
            <h4 class="text-lg font-black uppercase tracking-tight">ข้อมูลผู้จอง</h4>
          </div>
          <div class="flex items-center gap-4 p-4 bg-base-200/30 border border-base-content/5">
            <div class="avatar placeholder">
              <div class="bg-primary text-primary-content w-14 h-14 flex items-center justify-center shadow-lg shadow-primary/10 ring-2 ring-white">
                <span class="text-xl font-black">{{ booking.snap_user_name?.charAt(0) }}</span>
              </div>
            </div>
            <div class="space-y-1">
              <div class="text-xl font-black leading-tight">{{ booking.snap_user_name }}</div>
              <div class="flex flex-wrap gap-x-3 gap-y-1">
                <div class="flex items-center gap-1.5 text-[10px] font-bold opacity-50"><Mail :size="12" /> {{ booking.snap_user_email }}</div>
                <div class="flex items-center gap-1.5 text-[10px] font-bold opacity-50"><Briefcase :size="12" /> {{ booking.snap_user_department || 'N/A' }}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="space-y-3">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg bg-success/10 flex items-center justify-center text-success"><ShieldCheck :size="16" /></div>
            <h4 class="text-lg font-black uppercase tracking-tight">สถานะและการอนุมัติ</h4>
          </div>
          ...
          <div v-else class="py-8 text-center bg-warning/5 border border-dashed border-warning/20 flex flex-col items-center">
            <div class="w-14 h-14 bg-warning/10 rounded-full flex items-center justify-center text-warning mb-4 animate-pulse">
              <Clock :size="28" />
            </div>
            <h5 class="text-lg font-black text-warning uppercase">อยู่ระหว่างการพิจารณา</h5>
            <p class="text-[10px] opacity-40 mt-1">โปรดรอการตรวจสอบจากผู้ดูแลระบบ</p>
          </div>
        </section>
      </div>

      <!-- Footer Action -->
      <div class="p-6 md:p-8 bg-base-200/50 border-t border-base-content/5 flex flex-wrap justify-end gap-3 shrink-0">
        <!-- Approver Actions -->
